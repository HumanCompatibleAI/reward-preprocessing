...
if [[ $DRLHP == 1 ]]; then
  ...
  for path in $MODEL_PATHS; do
    ...
    echo "poetry run python -m reward_preprocessing.optimize_continuous with env.shaped_mountain_car $FAST model_path=results/preference_comparisons/$path/final_reward_net.pt save_path=processed/preference_comparisons/$path 'rollouts=$ROLLOUTS'" >> $cmds
  done
fi
...
if [[ $AIRL == 1 ]]; then
  ...
  for path in $MODEL_PATHS; do
    ...
    echo "poetry run python -m reward_preprocessing.optimize_continuous with env.shaped_mountain_car $FAST model_path=results/adversarial/$path/checkpoints/final/reward_test.pt save_path=processed/adversarial/$path 'rollouts=$ROLLOUTS'" >> $cmds
  done
fi
...
if [[ $TRUTH == 1 ]]; then
  ...
  for path in $MODEL_PATHS; do
    ...
    echo "poetry run python -m reward_preprocessing.optimize_continuous with env.shaped_mountain_car $FAST model_path=results/ground_truth_models/$path.pt save_path=processed/ground_truth/$path 'rollouts=$ROLLOUTS'" >> $cmds
  done
fi
