...
class SasRewardModel(RewardModel):
    """An MLP reward model with 2 hidden layers and (s, a, s') triples as input.
    Uses ReLU activation functions.
    ...
    Args:
        state_shape: shape of the observations the environment produces
            (these observations have to be arrays of floats, discrete
            observations are not supported)
        action_shape: shape of the observations the environment produces
            (these actions have to be arrays of floats, discrete
    ...
    def __init__(
        ...
        super().__init__(state_shape)
        num_features = 2 * np.product(state_shape) + np.product(action_shape)
        self.net = nn.Sequential(
            nn.Linear(num_features, hidden_size),
            nn.ReLU(),
            nn.Linear(hidden_size, hidden_size),
    ...
    def forward(self, transitions: Transition) -> torch.Tensor:
        ...
        x = torch.stack([transitions.state, transitions.next_state], dim=1)
        x = x.flatten(start_dim=1)
        actions = transitions.action.flatten(start_dim=1)
